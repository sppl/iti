Iticoin-qt: Qt4 GUI for Iticoin
===============================
...
Alternatively, install Qt Creator and open the `iticoin-qt.pro` file.

An executable named `iticoin-qt` will be built.
...
A warning for people using the *static binary* version of Iticoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Iticoin is linked against libdb5.3.
...
installed causes iticoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
