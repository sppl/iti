...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #iticoinTEST2\r");
            Send(hSocket, "WHO #iticoinTEST2\r");
        } else {
            // randomly join #iticoin00-#iticoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #iticoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #iticoin%02d\r", channel_number).c_str());
        }
